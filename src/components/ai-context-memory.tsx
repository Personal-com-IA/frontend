    ...
    return (
      <aside className="rounded-2xl border border-slate-800 bg-slate-900/40 p-4 backdrop-blur-xl h-full flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-[10px] font-mono text-slate-500">Sincronizando Memória...</span>
        </div>
      </aside>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar">
        {learningHistory.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 text-center opacity-50">
            ...
            <p className="text-xs text-slate-500">O banco de episódios está vazio.</p>
              ...
              <div className="mb-2 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                  <div className={`h-1.5 w-1.5 rounded-full ${
                    entry.topic?.includes("NEGATIVO") ? "bg-red-400" : "bg-yellow-400"
                  }`} />
